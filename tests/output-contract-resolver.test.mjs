...
function addPackRegistryViews(root) {
  ...
  const packTemplatePath = path.join(root, "pack/document-templates/pack-summary.json");

  mkdirSync(path.dirname(outputsRegistryPath), { recursive: true });
  writeJson(packTemplatePath, {
    schemaVersion: 1,
    kind: "pack-summary",
    status: "ready",
    summary: "<summary>",
...
test("resolves pack output from generated registry views without changing core defaults", () => {
  withTempRoot((outputs) => outputs, (root) => {
    ...
    assert.equal(result.template, path.join(root, "pack/document-templates/pack-summary.json"));
    assert.equal(result.absoluteTemplatePath, path.join(root, "pack/document-templates/pack-summary.json"));
  });
});
