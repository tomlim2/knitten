...
- **Accidental inline tags — NEVER write bare `#NNN` or `#word` in body prose.** Obsidian treats any whitespace-prefixed `#text` as a tag and pollutes the tag pane. Cases that bite: PR/issue numbers (`#154`), review finding references (`#1`, `#2`), hex colors (`#1a1c2c`), checklist items (`#TODO`). Fixes in priority order:
  - **Plain prose labels** for review findings: `Finding 1`, `case 2`, `item 3`; never `#1`.
  - **PR/issue text without `#`** for private work: `PR 154`, `issue 154`; do not link private repository UI URLs.
