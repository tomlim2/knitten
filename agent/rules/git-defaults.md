...
- **Local report inbox exception** — If the task is only to leave a report-only handoff, do not run `scripts/worktree-start.mjs` and do not create a branch or worktree. Write transient cross-session reports under gitignored `.agent-local/reports/`. If the report becomes durable policy, a spec, a decision, or an accepted finding, move it to the tracked owner path and use the normal worktree flow.
