...
- **Documentation language** — Operational docs (skill `SKILL.md`, rule `rules/*.md`, standard `standards/*.md`) are **English only**. Reason: LLM tokenizers are more efficient on English; Korean burns more tokens for the same content, and operational docs are read on every cold-start.
