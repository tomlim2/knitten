...
Canonical key registry for `~/.claude/private/agent-hub-config/repo-paths.json`. Each machine registers its own paths, but keys MUST match this list.

Read on-demand: before `/ah-manage-config`, repo-path setup, or setting up a new machine.

---

## Required Keys

Keys that skills/commands depend on. Register with `/ah-manage-config add repo-paths <key> <path> [description]`.
...
| `agent-hub` | Agent hub repository | git | ah-check-updates, ah-sync-vendors, context routing, git-pull-repos |
...
Some logical names resolve to different physical keys depending on which machine is running. `obsidian`, for example, can be a staging vault on the work machine and a different vault on a home machine. This is handled by a small alias layer in `~/.claude/private/agent-hub-config/hardware.json`:
...
1. Read `~/.claude/private/agent-hub-config/hardware.json`. If `aliases[<logical>]` exists, use that value as the repo-paths key.
...
3. Read `~/.claude/private/agent-hub-config/repo-paths.json` and look up the chosen key to get the absolute path.
...
4. Run `/ah-manage-config validate` to verify every alias resolves to an existing directory on this machine.
