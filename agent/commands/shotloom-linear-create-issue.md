...
- **Shotloom Linear 이슈에 개인 private 레포를 절대 링크/언급하지 말 것.** 포함: `bevy-vrm`, `anju`, `mmd-anju`, `ta-portfolio`, `StoryPreviz`, 그 외 `~/.claude/private/agent-hub-config/repo-paths.json`에 등록되었지만 CINEV 소유가 아닌 모든 레포.
