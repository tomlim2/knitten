...
```
To register missing refs: `/ah-manage-config add repo-paths <name> <path>`.
```
...
- Read `~/.claude/commands/ah-review-claude-md.md`
...
Launch a Task subagent:
- Read `~/.claude/commands/ah-update-skills.md`
...
Launch a Task subagent:
- Read `~/.claude/commands/ah-review-skills.md`
...
3. **Exclude `agent-hub`** — entry documents are handled by [1/4]
4. For each remaining connected repo, launch a Task subagent:
   - Read `~/.claude/commands/ah-update-docs.md`
