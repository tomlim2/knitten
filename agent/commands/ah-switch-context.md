...
1. `~/.claude/private/agent-hub-config/repo-paths.json` — repo paths (each entry: `{ path, description }` or plain string)
2. `~/.claude/private/agent-hub-config/contexts.json` — personal projects (fallback to defaults below if missing)
...
```bash
tutoring_root=$(bash ~/.claude/skills/ah-resolve-doc-path/resolve.sh doc tutoring | awk -F= '/^RESOLVED_PATH=/{print $2; exit}')
consulting_root=$(bash ~/.claude/skills/ah-resolve-doc-path/resolve.sh doc consulting | awk -F= '/^RESOLVED_PATH=/{print $2; exit}')
```
