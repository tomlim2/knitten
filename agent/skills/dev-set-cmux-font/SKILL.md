...
- `SF Mono` (preinstalled on macOS, `!`bash ~/.claude/skills/ah-resolve-doc-path/resolve.sh tool font-sf-mono``)
...
If Sarasa Mono isn't installed yet, install with `brew install --cask font-sarasa-gothic` (confirm with user first). The cask ships a single `!`bash ~/.claude/skills/ah-resolve-doc-path/resolve.sh tool font-sarasa`` that contains every regional variant.
