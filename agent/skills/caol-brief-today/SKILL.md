...
1. **Obsidian 가용성** — `repo-paths.json`의 `obsidian` 키:
   - 존재 → `obsidianAvailable = true`, `obsidianClaudeDir = {obsidian}/claude`
   - 없음 → `obsidianAvailable = false`. **정상 상태** (회사 머신은 iCloud 없음). 에러 띄우지 말 것.
2. Active context:
   - `obsidianAvailable`면 `{obsidianClaudeDir}/contexts.json` → `active`
   - 아니면 `~/.claude/private/active-context.json` → `active` (fallback)
   - 둘 다 없으면 `회사` default.
3. `--context`가 있으면 덮어씀.
4. 대상 레포:
   - **회사**: shotloom, cinev-studio, cinev-engine, cinev-ta-tools (존재하는 것만)
   - **개인**: CINEV 소속 아닌 레포 (anju, bevy-vrm, ta-portfolio, mmd-anju 등)
   - **부업**: `tutoring/lessons/`, `consulting/` 스캔. `obsidianAvailable == false`면 "vault 없어서 스킵" 한 줄.
...
4. **미정리 어제 일지** (obsidianAvailable gate) — `find obsidian-staging -mtime -2`. false면 섹션 완전 생략.
...
2. **Gate (3E.2/3/4)**: `obsidianAvailable == true`일 때만. false면 3개 섹션 완전 생략, 3E.1만 + "obsidian 없는 머신" 각주.
3. **obsidian-staging 승격 후보** — `find obsidian-staging -newermt "<today>"`. H1 추출, 프로젝트 추측, 승격 경로 제안.
4. **중복 문서 감지** — 최근 7일 문서 파일명 normalize 비교 (1차), H1 Levenshtein ≤3 비교 (2차 필요 시). 본문 diff 안 함.
5. **오래된 staging** — `find obsidian-staging -mtime +7` → `/learn-archive-week` 또는 삭제 제안.
...
저장 경로: `{obsidianClaudeDir}/briefings/{YYYY-MM-DD}.md`
...
- **Obsidian 없는 머신이 normal** — 회사 머신은 iCloud 동기화 없음. obsidian-staging / 중복 감지 / briefings 저장 스킵은 정상. "등록 필요"/"미비점" 같은 경고 띄우지 말 것.
