...
```
python ~/.claude/skills/cci-sync-ta-tools/sync.py                        # dry-run preview
python ~/.claude/skills/cci-sync-ta-tools/sync.py --execute              # apply changes
python ~/.claude/skills/cci-sync-ta-tools/sync.py --direction anju       # anju → ta-tools only
python ~/.claude/skills/cci-sync-ta-tools/sync.py --direction ta-tools   # ta-tools → anju only
```

## Behavior

1. Reads repo paths from `~/.claude/private/agent-hub-config/repo-paths.json`
...
When the user invokes `/cci-sync-ta-tools`:
