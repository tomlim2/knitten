...
```
!`bash ~/.claude/skills/ah-resolve-doc-path/resolve.sh doc tutoring`
├── invoices/
...
```
!`bash ~/.claude/skills/ah-resolve-doc-path/resolve.sh doc drinks`
└── drinks.json
```
...
```bash
tutoring_dir="$(bash ~/.claude/skills/ah-resolve-doc-path/resolve.sh doc tutoring | awk -F= '/^RESOLVED_PATH=/{print $2}')"
