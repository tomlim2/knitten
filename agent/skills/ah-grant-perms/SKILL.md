...
**User-only — the model must not auto-invoke this** (`disable-model-invocation: true`). Treat as security-sensitive: a granted state lets every subsequent tool call run without approval until `/ah-revoke-perms`.

## Arguments

None.

Usage: `/ah-grant-perms`
...
state=$state_dir/ah-perms-grant.json
...
if [[ -f "$state" ]]; then
  ...
  cat "$state"
  echo
  echo "Run /ah-revoke-perms first if you want to re-grant."
  exit 1
fi
```
...
⚠️  YOLO mode is active. Every tool call now runs without approval prompts.
   Revoke with: /ah-revoke-perms
```

## Notes

- **Idempotent guard:** if `ah-perms-grant.json` already exists, refuse. Stops accidental double-grant which would lose the original backup pointer.
...
- **Why disable-model-invocation:** The model must never auto-grant itself elevated permissions. Only the user types `/ah-grant-perms`.
- **Restart not required:** The harness re-reads settings.json on next tool dispatch; grant takes effect immediately.

## Related

- `~/.claude/skills/ah-revoke-perms/SKILL.md` — restore from backup
