---
description: "Collect and save macOS hardware specs (GPU, RAM, chip) to private/agent-hub-config/hardware.json for capability checks."
allowed-tools: Bash(system_profiler:*), Bash(sw_vers:*), Write, Read
---

# system-save-hardware

Collect macOS hardware specs and save to `~/.claude/private/agent-hub-config/hardware.json`.
---
...
Before writing, read the existing `~/.claude/private/agent-hub-config/hardware.json` if it exists. Other skills may have added non-hardware machine-local keys (e.g. `aliases`, future policy blocks) that must survive a hardware refresh.
...
Write the merged JSON to `~/.claude/private/agent-hub-config/hardware.json`:
...
- **File:** `~/.claude/private/agent-hub-config/hardware.json`
...
```markdown
Read `~/.claude/private/agent-hub-config/hardware.json` to check machine specs.
```
