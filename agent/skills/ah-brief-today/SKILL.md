...
Morning / evening briefing for the currently active work context (회사/개인/부업). Time-aware: morning mode plans the day, evening mode recaps, midday is a tight status line.
...
활성 컨텍스트는 `ah-switch-context`가 설정한 값. 회사 컨텍스트면 shotloom + CINEV 레포만.
...
1. **Obsidian 가용성** — `~/.claude/private/agent-hub-config/machine-paths.json`의 `obsidian` 키:
...
저장 경로: `bash ~/.claude/skills/ah-resolve-doc-path/resolve.sh doc daily`로 얻은 `daily/` 아래 `{YYYY-MM-DD}.md`.
...
- `ah-switch-context` — active context 결정
- `ah-check-status` — skill server 상태
