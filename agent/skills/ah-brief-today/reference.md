# ah-brief-today reference

Output templates and detail commands for the ah-brief-today skill. SKILL.md holds the workflow skeleton and critical rules (KST, obsidian gate); this file holds the concrete commands and example output blocks.
...
```bash
staging="$(jq -r '."obsidian-staging" // empty' ~/.claude/private/agent-hub-config/machine-paths.json)"
...
```bash
vault="$(jq -r '."obsidian" // empty' ~/.claude/private/agent-hub-config/machine-paths.json)"
