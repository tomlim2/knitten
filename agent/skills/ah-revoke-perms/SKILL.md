...
Close down YOLO mode — restore `~/.claude/settings.json` from the backup created by `/ah-grant-perms`, then delete the grant state file. Idempotent: if no grant is active, says so and exits.

**User-only — the model must not auto-invoke this** (`disable-model-invocation: true`). Treat as the safety partner of `/ah-grant-perms`.

## Arguments

None.

Usage: `/ah-revoke-perms`
...
```bash
state=~/.claude/private/ah-perms-grant.json
...
- **No-op when not granted** — running `/ah-revoke-perms` without a prior grant just prints "Nothing to revoke" and exits 0. Safe to run blind.
...
| Symptom | Fix |
|---------|-----|
| `state` file present but `backup` file gone | Manual: edit `~/.claude/settings.json` directly to remove the YOLO permissions, then `rm ~/.claude/private/ah-perms-grant.json`. |
...
- `~/.claude/skills/ah-grant-perms/SKILL.md` — the partner skill that creates the grant state
