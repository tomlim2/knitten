...
- one beat purpose per unit, usually problem, cause clue, corrective action,
  and resolved consequence for four-panel work;
- a state-timing table for each repeated object or path token, showing what it
  knows before, during, and after the visible corrective action;
...
Never let a later result appear as a finished visual state before the panel
whose visible action causes it. For example, a safe path must not already form
before the sign, gate, bridge, or other causal object is repaired or moved.
...
reader causality, state timing without anticipation, and actor/object
relationship legibility. Adaptation gets at most two storyboard attempts. If
the second fails, return `blocked: storyboard-validation-failed`; Output must
not run.
