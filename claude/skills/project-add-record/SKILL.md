...
Replace `$COMMAND_NAME` with: `project-add-record`

## Purpose

프로젝트나 태스크를 끝낸 후 기록용으로 사용. 코드베이스를 자동 탐색해서 기술 스택, LOC, 모듈 구조, 핵심 기법 등을 추출하고 구조화된 기록을 남긴다. 이후 포트폴리오, 이력서, 회고 등에 재활용 가능.

---

## Arguments

- `<project-path-or-repo-name>` - 프로젝트 경로 또는 `repo-paths.json`에 등록된 이름
- `[subfolder]` - 특정 하위 폴더만 대상 (예: `web/matcap-painter`)
...
**기록 저장:**
```
{obsidian-vault}/claude/projects/records/{project-name}.md
```
...
| 용도 | 경로 |
|------|------|
| **기록 (항상)** | `{obsidian}/claude/projects/records/{project-name}.md` |
| **포트폴리오 (선택)** | `{obsidian}/claude/projects/ta-portfolio-content-design.md` |
