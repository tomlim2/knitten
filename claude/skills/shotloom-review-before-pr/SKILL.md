...
> **Self-review's blind spot**: when the same model that wrote the prose is also reviewing it, automated grep sweeps (A1, D1–2, H1, H8, H9) stay objective but **semantic-judgment checks (H10 cross-ref accuracy, H11 sibling-state claims, the absolutist-vs-factual call) silently revert to author-validation**. The grep finds the keyword; the author rationalizes the keyword as "factual" because they remember writing it that way. For load-bearing prose claims (architecture docs, ADR cross-refs, factual statements about sibling crates), dispatch an independent Explore subagent with zero context about the edit — pass only the diff range, the rules, and the verification list. The agent's judgment is the only objective one available.
...
- **H8** — `STL-NN` ticket references in source code, comments, module docs, doc-strings, or README prose. Linear ticket IDs belong in commit messages and PR descriptions only — they rot fast (renumber, cancel, repurpose) and they tie code rationale to a tracker the next reader may not be able to open. Rewrite to describe the *what* (the rule, the invariant, the algorithm) and let the commit/PR carry the *who-asked-for-it*. Sweep added lines for `\bSTL-\d+\b` occurrences in `*.rs` / `*.md` / `*.toml` / `*.json` and flag every hit. Exception: ADR Status / Amendment blocks (per H7) and CHANGELOG-style files explicitly designed to track tickets.
- **H9** — past-state contrast framing in added comments. Phrasings like `previously …`, `prior pipeline`, `was unconstrained`, `cannot resolve` (when describing a sibling pass to imply this one fixes it), `used to`, `before this change`, `now …` describe the *delta* rather than the *current state*. The comment lives forever in the source; the contrast becomes meaningless once the reader doesn't remember the prior version. Rewrite to describe what the symbol IS, what it does, and the invariant it pins — drop the implicit comparison to whatever the diff replaced. Sweep added lines for the keyword set above and flag every hit. Exception: changelog entries, commit messages, PR descriptions, ADR Amendment blocks — those exist precisely to record the delta.
- **H10** — cross-reference fabrication. When added prose cites an in-repo location by section name (`ADR-NNNN §SectionName`, `docs/arch/foo.md §Bar`, `<crate>/README.md §Baz`), open the cited file and verify (a) the section header exists at the named path, AND (b) the section's body actually contains the material the prose claims is there. H5 only verifies (a) — H10 closes the gap on (b). The failure mode: the section exists, the author skim-reads it as supporting their claim, but the section's actual text says something different (or weaker, or about a sibling concern). Rephrase using the cited section's literal phrasing ("formalizes in the implementation PR") rather than a paraphrase the next reader cannot reconcile against the source ("deferred", "locked against"). Treat as P2 when the cited section anchors a load-bearing architectural claim; nit otherwise — but still flag, because paraphrase drift hides the misalignment.
- **H11** — sibling-state hallucination. When added prose makes a factual claim about another crate's state (`scaffold`, `stub`, `WIP`, `empty`, `owns X`, `produces Y type`, `no output yet`, `not used by anything`, `the only consumer is Z`), open that crate's `lib.rs` / `types.rs` / `Cargo.toml` and verify against current code. Author-side self-review repeatedly fails this check because the author judges their own confidence in the claim instead of re-reading the cited crate. The failure mode that motivated this sweep: a normalizer-pipeline asymmetry table cell described `shotloom-facial-anim-normalizer` as "scaffold; no output type yet" when the crate already exported `ExpressionTrack` and ran a config-driven channel remap. Self-review reported clean (A–H all green); the made-up state survived to push and was caught only by an independent fresh-eyes pass. Treat as P1: factual errors in user-facing architecture docs become source-of-truth for future maintainers. **Verification rule**: every added line containing the keyword set above (case-insensitive) requires a one-line evidence quote from the cited crate's actual source pasted next to the finding — no quote, no clean.
- **H12** — ADR execution-status leak. ADRs are decision records, not implementation logs. AFDS `documentation-standard.md` §5.7 explicitly excludes "active execution status" from `docs/adr/*.md`. Sweep added lines under `docs/adr/*.md` (NOT just `adr-NNNN-*.md` — also `README.md` index entries) for: `Implementation status`, `Implementation log`, `landed in`, `this PR scopes`, `this PR locks`, `STL-\d+ \(this PR\)`, `formalized by PR #\d+`, `as of <date>`, `currently scoped to`. Right home for the same content: `crates/<crate>/README.md` (per-module README) for crate-internal scope, `CHANGELOG.md` for release-visible deltas, the commit body or PR description for PR-local context. Do not extend the ADR body with "Implementation status (X)" subsections, even inside `§Out of scope` or `§Consequences` — those sections record durable architectural reasoning, not landing notes. Treat as P2 when the leak is in a Decision-adjacent section; nit when it's in a peripheral block. The failure mode that motivated this sweep: an in-place edit added `**Implementation status (character model):** STL-216 (this PR) scopes ...` to ADR-0030 §"Out of scope". Two separate self-review passes reported clean; round-1 reviewer flagged it as ADR-misuse with one sentence ("ADR 은 구현 기록이 아닙니다").
- **H13** — per-module rationale in `//!` instead of crate README. Long rationale prose at the top of `lib.rs` / module file — `# Field semantics`, `# Coordinate-system asymmetry`, `# Algorithm`, `# Why X works`, `# extract vs normalize`, `# Heuristic origin` — belongs in `crates/<crate>/README.md` per AFDS §5.13 ("per-module README owns tricky algorithms, non-obvious invariants, and implementation constraints"). The module-doc comment is for purpose + public-surface pointer + boundary statement; the README is for the why and the gotchas. **Heuristic for the sweep**: any added `//!` block where (a) the block exceeds ~30 lines OR (b) carries 3+ `# Heading` sections of explanatory prose AND (c) the crate has a `README.md` with similar prose layout, the rationale should relocate to README. Module-doc keeps a one-paragraph pointer ("See the crate README's [`Section A`] / [`Section B`] sections for X"). Treat as P2 when the relocated content is multi-section; nit when one short section. The failure mode that motivated this sweep: foot_geometry.rs `//!` carried four expanded sections (extract-vs-normalize, field semantics, coord asymmetry, sole_offset accuracy) totaling ~75 lines — round-1 reviewer asked to relocate to the existing crate README.
- **H14** — numeric / geometric claim not derived from code. When prose makes a claim about what a numeric constant means geometrically or semantically (`0.6 ratio is forward of the toe pad`, `the 1.0 represents the bone tip`, `index 5 is the wrist`), open the code that uses the constant and verify the claim against the actual arithmetic. Authors write these claims from intuition / mental-model and skip the arithmetic check; reviewers and AI deep reviews catch them later. The failure mode that motivated this sweep: docs claimed `toe_tip_z = 0.6 * |foot_to_toes_z|` was "the visible foot sole's leading edge (front of the toe pad)", but `0.6 < 1.0` (where 1.0 is the toes-bone position) means the value sits BETWEEN foot bone and toes bone — i.e. the metatarsal / ball-of-foot region, NOT past the toes bone. The author wrote "front" / "leading edge" from intuition without doing the `0.6 < 1.0` arithmetic. AI Deep Review caught it as MEDIUM in round 1. **Verification rule**: every added prose line that pairs a literal numeric constant (`0.4`, `0.6`, `1.0`, `5`, `52`) with a directional/positional claim (`forward of`, `behind`, `past`, `before`, `at`, `between X and Y`) requires a sanity-derivation either in the same comment ("0.6 < 1.0, so between foot and toes") or in a code-context line reference. No derivation, no clean.

Mindset: **doc must describe what IS, not what MIGHT BE, not what WAS, not what the author REMEMBERS, not what the author INTUITS without checking.** Memory-driven prose about sibling crates rots fastest; intuition-driven prose about numeric constants is wrong fastest. H10/H11 require literal verification of a cited file; H14 requires literal arithmetic verification of a cited constant. ADRs are decisions (H7/H12); implementation status / execution log goes elsewhere. Long rationale (H13) goes to per-module README, not module-doc. Default verdict on any H finding: rewrite to current-state-only, derived-from-source, located-where-AFDS-says.
