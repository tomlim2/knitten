...
Mindset: **doc must describe what IS, not what MIGHT BE, not what WAS, not what the author REMEMBERS.** Memory-driven prose about sibling crates rots fastest because the reader has no signal to check against. H10/H11 require literal verification — open the cited file, paste a quote, then judge. Comments that promise future work without a concrete issue ID are wishes, not specification. Comments that claim "scaffold" after 200 lines of logic land are lies. Comments that frame the symbol as "fixes what the prior pass cannot do" become noise once the prior pass is forgotten. ADRs that carry execution-plan prose are progress trackers, not durable rationale. Default verdict on any H finding: rewrite to current-state-only.
