...
Verify:
- cwd is `/Users/deemooooooooo/Desktop/www/shotloom-github` (or reported toplevel matches)
...
3. Determine **worktree base** directory:
   ```bash
   repo_root=/Users/deemooooooooo/Desktop/www/shotloom-github
   if grep -qE '^\.?worktrees/?$' "$repo_root/.gitignore" 2>/dev/null; then
     ...
     worktree_base="/Users/deemooooooooo/Desktop/www/shotloom-worktrees"
     mkdir -p "$worktree_base"
   fi
   ```
   For Shotloom today this resolves to `/Users/deemooooooooo/Desktop/www/shotloom-github/.worktrees/`.
...
- The Ready briefing is the **only** output. No code, no plan, no extra prose until user confirms.
- If the user explicitly says "skip pre-flight" or "i already did this", log that decision and skip Step 3 only — never skip Step 1.
