...
1. `~/.claude/private/repo-paths.json` 읽기
...
1. `~/.claude/private/repo-paths.json`에서 `obsidian` 경로 읽기
2. `{obsidian}/claude/projects/{project-name}/` 디렉토리 생성
3. 기본 파일 생성:
...
- 스킬 연동: {스킬명 / 없음}
- Obsidian: claude/projects/{project-name}/
  ├── devlog.md
  └── learnings-index.md
```
