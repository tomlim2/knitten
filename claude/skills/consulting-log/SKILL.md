...
Replace `$COMMAND_NAME` with: `consulting-log`
...
```
Usage:
  /consulting-log CompanyName         → Log a new session for CompanyName
  /consulting-log list                → Show all companies with session counts
  /consulting-log summary             → Overview of all consulting activity
```
...
- For portfolio use, run `/consulting-log summary` to get exportable data
