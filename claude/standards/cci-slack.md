...
Shared across all cci Slack skills and the MCP server.
...
- **Created by:** `cci-art-create-branch` (via MCP `thread_save`)
- **Read by:** `cci-art-send-merge-notice`, `cci-art-send-merge-result` (via `send.py`)
...
Saved by `cci-art-send-merge-result` before sending messages. Korean PM-friendly summaries, no contributor info:
...
|-------|------|-----------|
| `cci-art-create-branch` | Create branch, send announcement, save thread | MCP `slack_post_message` + `thread_save` |
| `cci-art-send-notice` | Send general messages to art channel | MCP `slack_post_message` |
| `cci-art-send-merge-notice` | Send merge warning as thread reply | `send.py` |
| `cci-art-send-merge-result` | Send merge completion + detail as thread replies | `send.py` |

---

## MCP Server

Registered as `cci` MCP server:
...
1. **Use `send.py` for threaded replies** — Skills with `send.py` (`cci-art-send-merge-notice`, `cci-art-send-merge-result`) MUST use their script. Direct API calls or MCP tools may miss thread context.

2. **Never use Claude AI Slack MCP tools** — Tools like `slack_send_message` from Claude AI's built-in Slack integration show "Sent via @Claude" and wrong sender identity. Always use `send.py` or the `cci` MCP server.
...
5. **Save stats before sending** — `cci-art-send-merge-result` must save merge stats to `private/art-merge-stats/` before any Slack delivery.
