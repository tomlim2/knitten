...
- **Drinks database**: `{vault_claude}/drinks/drinks.json` where `{vault_claude}` is `jq -r '."obsidian-vault-claude"' ~/.claude/private/caol-config/machine-paths.json` (falls back to `obsidian-staging` key on machines without an iCloud vault)
...
Read `{vault_claude}/drinks/drinks.json` and display all drinks in a table:
...
1. **Check existing database**
   - Read `{vault_claude}/drinks/drinks.json`
...
5. **Save to database**
   - Create `{vault_claude}/drinks/` directory if needed
