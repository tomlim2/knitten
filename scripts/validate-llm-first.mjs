...
import { mkdtemp, readdir, readFile, rm, stat, access } from "node:fs/promises";
...
async function checkPublicSafetyScrubGates() {
  const violations = [];
  let inventory;
  try {
    inventory = await readJsonRel("agent/config/artifact-inventory.json");
  } catch (err) {
    violations.push({
      file: "agent/config/artifact-inventory.json",
      line: 1,
      message: `cannot read artifact inventory: ${err.message}`,
    });
    return { name: "public-safety-scrub-gates", violations };
  }
  ...
  for (const row of inventory.rows) {
    ...
    if (row["privacy-risk"] === "public-safe") continue;
    violations.push({
      file: row["source-artifact-path"] || "agent/config/artifact-inventory.json",
      line: 1,
      message: `${row["row-id"] || "(missing row-id)"} privacy-risk ${row["privacy-risk"]} blocks public core promotion`,
    });
  }
...
async function checkCoreReleaseValidation() {
  ...
  const violations = [];
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), "knitten-core-release-"));
  const outputDir = path.join(tempRoot, "core");
  ...
  try {
    ...
    for (const file of requiredFiles) {
      if (existsSync(path.join(outputDir, file))) continue;
      violations.push({
        file,
        line: 1,
        message: "required public release file is missing from assembled core tree",
      });
    }
  } catch (err) {
    violations.push({ file: manifestPath, line: 1, message: `release tree assembly failed: ${err.message}` });
  } finally {
    await rm(tempRoot, { recursive: true, force: true });
  }

  return { name: "core-release-validation", violations };
}

async function checkKnittenCoreTreeSmoke() {
  const manifestPath = "docs/plans/reports/knitten-pluginization-core-extraction/knitten-core-export-manifest-2026-06-01.json";
  const violations = [];
  let manifest;
  try {
    manifest = await readJsonRel(manifestPath);
  } catch (err) {
    violations.push({ file: manifestPath, line: 1, message: `cannot read manifest: ${err.message}` });
    return { name: "knitten-core-tree-smoke", violations };
  }

  if (!Array.isArray(manifest.exports)) {
    violations.push({ file: manifestPath, line: 1, message: "exports must be an array" });
    return { name: "knitten-core-tree-smoke", violations };
  }

  const tempRoot = await mkdtemp(path.join(os.tmpdir(), "knitten-core-smoke-"));
  const outputDir = path.join(tempRoot, "core");
  try {
    await execFileAsync(
      process.execPath,
    ...
    for (const entry of manifest.exports) {
      const targetPath = entry["target-path"] || entry["source-artifact-path"];
      if (!targetPath) {
        violations.push({ file: manifestPath, line: 1, message: `${entry["row-id"] || "(missing row-id)"} missing target path` });
        continue;
      }
      if (!existsSync(path.join(outputDir, targetPath))) {
        violations.push({ file: targetPath, line: 1, message: "manifest target missing from assembled tree" });
      }
    }

    if (!existsSync(path.join(outputDir, "README.md"))) {
      violations.push({ file: "README.md", line: 1, message: "public README overlay missing from assembled tree" });
    }

    for (const deferred of ["AGENT-HUB.md", "LOOKUP.md", "CHANGELOG.md"]) {
      if (existsSync(path.join(outputDir, deferred))) {
        violations.push({ file: deferred, line: 1, message: "deferred private-root file appears in assembled tree" });
      }
    }
  } catch (err) {
    violations.push({ file: manifestPath, line: 1, message: `assembly failed: ${err.message}` });
  } finally {
    await rm(tempRoot, { recursive: true, force: true });
  }
...
async function checkArtifactPackRegistryMerge() {
  const violations = [];
  try {
    await execFileAsync(process.execPath, ["--test", "tests/artifact-pack-registry-merge.test.mjs"], {
      cwd: REPO_ROOT,
      encoding: "utf8",
      maxBuffer: 20 * 1024 * 1024,
    });
    ...
    violations.push({
      file: "tests/artifact-pack-registry-merge.test.mjs",
      line: 1,
      message: `artifact pack registry merge tests failed: ${(err.stdout || err.stderr || err.message).trim()}`,
    });
  }
...
const CHECKS = [
  ...
  { name: "public-safety-scrub-gates", fn: checkPublicSafetyScrubGates, full: false },
  { name: "core-release-validation", fn: checkCoreReleaseValidation, full: false },
  { name: "knitten-core-tree-smoke", fn: checkKnittenCoreTreeSmoke, full: false },
