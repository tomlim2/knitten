...
function candidateExactNames(candidate) {
  if (candidate["compatibility-need"] === "none") {
    return [candidate["artifact-id"], candidate["candidate-id"]].filter(Boolean);
  }
  const oldName = candidate["compatibility-old-name"] === candidate["artifact-id"]
    ? undefined
    : candidate["compatibility-old-name"];
  return [
    candidate["candidate-id"],
    candidate["compatibility-alias-id"],
    oldName,
    candidate["compatibility-old-path"],
    candidate["compatibility-shim-path"],
  ].filter(Boolean);
}

function exactNameMatchValue(candidate, request) {
  const named = asArray(request.namedArtifact).filter(Boolean);
  const text = request.requestText || "";
  return candidateExactNames(candidate).find((name) => named.includes(name) || textHasNameToken(text, name));
}

function exactNameMatches(candidate, request) {
  return Boolean(exactNameMatchValue(candidate, request));
}
...
function routeEvidence(candidate, request) {
  ...
  const matchedName = exactNameMatchValue(candidate, request);
  if (matchedName) {
    evidence.push({ axis: "exact-name", value: matchedName });
  }
...
function candidateFromIndex(row, indexEntry, manifest, compatibility = null) {
  ...
  const candidate = {
    ...
    "compatibility-old-name": compatibility?.["old-name"],
    "compatibility-old-path": compatibility?.["old-path"],
    "compatibility-shim-path": compatibility?.["shim-path"],
...
function compareCandidates(a, b) {
  ...
  if ((b["exact-match-rank"] || 0) !== (a["exact-match-rank"] || 0)) return (b["exact-match-rank"] || 0) - (a["exact-match-rank"] || 0);
...
function ambiguousTie(candidates) {
  ...
  if ((first["exact-match-rank"] || 0) !== (second["exact-match-rank"] || 0)) return null;
...
export function resolveArtifactRoute(input) {
  ...
  for (const entry of input.coreArtifacts || []) {
    ...
    const exactEvidence = evidence.find((item) => item.axis === "exact-name");
    candidate["route-evidence"] = evidence;
    candidate["route-score"] = evidence.filter((item) => item.axis !== "exact-name").length;
    candidate.exactMatch = Boolean(exactEvidence);
    candidate["exact-match-rank"] = exactEvidence ? String(exactEvidence.value).length : 0;
  ...
  for (const row of input.installedPacks || []) {
    ...
    for (const candidate of candidatesFromRow(row, manifest)) {
      ...
      candidate.exactMatch = Boolean(exactEvidence);
      candidate["exact-match-rank"] = exactEvidence ? String(exactEvidence.value).length : 0;
      if (candidate.exactMatch && candidate["compatibility-need"] !== "none") {
        candidate["matched-compatibility-input"] = exactEvidence.value;
      }
