...
const CASES_PATH = "evals/routing-smoke/cases.json";
const SURFACES_PATH = "evals/routing-smoke/activation-surfaces.json";
const REPORT_PATH = ".agent-local/ah/evals/routing-smoke/latest.json";
...
function usage() {
  return `Usage:
  run-routing-smoke-eval.mjs [--report] [--print-json]

Runs the deterministic KC routing smoke eval.`;
}
...
function runEval({ writeReport }) {
  ...
  for (const item of cases) {
    ...
    caseResults.push({
      ...
      loadedReferences,
      routeCorrect: predictedSkill === item.expectedSkill,
  ...
  const routingCorrect = caseResults.filter((item) => item.routeCorrect).length;
  ...
  if (routingCorrect < THRESHOLDS.routingCorrect) blockers.push(`routing accuracy ${routingCorrect}/20 below ${THRESHOLDS.routingCorrect}/20`);
  ...
  const report = {
    ...
    metrics: {
      ...
      routingAccuracy: { correct: routingCorrect, total: 20 },
