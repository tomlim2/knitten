---
status: accepted
---
# Briefings

Persistent task briefings live here.

| Path | Use |
|---|---|
| `shotloom/<slug>.md` | Shotloom Linear briefing written by `/shotloom-start-task` before `/shotloom-draft-spec` authors and reviews the spec. |
| `specs/<slug>.md` | High-risk agent-hub spec intake: user request, route, evidence, decisions, exclusions, and validation expectations before `ah-manage-spec` authors the direct spec. |

Rules:

- Write a briefing before writing the matching spec.
- Use the lifecycle spec path under `docs/plans/` for the matching spec.
- Commit the briefing with the matching spec when the direct spec lands.
