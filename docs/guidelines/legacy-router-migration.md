...
| Payload leaf | one concrete action after classification | compact activation shell |
...
## Payload Plugin Checklist For Migration

When migrating a payload plugin that still has router-shaped surfaces:
...
## Mechanical Workflow Index Script

Legacy router-shaped payloads should keep classification policy in a script, not
prose. The script must return machine-readable JSON and have fixtures or
validator coverage.

Recommended path:

```text
skills/<domain>-references/scripts/<domain>-workflow-index.mjs
```
...
the classification source of truth. Leaf skills should not read a parent script
or map to decide whether they apply.
...
2. Update internal references and mechanical workflow-index scripts.
...
Use the payload's own activation or workflow-index validator when it has one.

## Review Checklist

Before committing legacy-router migration changes, check:

- Does Core still avoid domain behavior?
- Does the payload plugin own its domain skills and references?
- Is there exactly one obvious workflow-index helper for a multi-leaf domain?
- Can the helper reject non-domain requests without reading leaf details?
...
- Are classification decisions in scripts and long workflows in references, not active
  skill bodies?
