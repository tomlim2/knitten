...
| Stage Import debug UI | `apps/editor/src/components/debug/StageImportDebugPanel.tsx` | Partial | UI exposes three map buttons but keeps them disabled while waiting for STL-423/STL-424 commands. |
| Map document contract | `contracts/stage-map/`, `docs/specs/stage-map-document.md` | Missing | Not present on `origin/main`; sibling STL-421 draft defines expected `background_owner` semantics. |
| Roadmap doc | `docs/roadmap/single-stage-import.md` | Missing | Linear cites it, but the file is absent on current `origin/main`. |
| STL-422 parser/resolver | `crates/`, `apps/`, `contracts/` search | Missing | No parser output type or resolver API is available on current `origin/main`. |
...
2. **The command consumes pre-resolved placement DTOs, not raw map documents.**
   ...
   current `origin/main`. STL-423 can still define the engine bridge boundary by
   accepting a list of already-resolved prop asset ids plus authored transforms
   and ownership tags. That keeps this PR reviewable and lets STL-422 connect
   later without duplicating parser logic.
...
3. **Use `ModelTransform` for document placement on the wire.**
   ...
   makes tests assert the final persisted prop transform directly.
...
4. **Store ownership as `PropModel.tags` in this PR.**
   ...
   avoids a domain-model schema expansion before the stage-map contract lands.
   The command must add deterministic tags such as `background_map`,
   `map:<map_id>`, and `source:stage_import_debug` so STL-424 can filter by
   tags without deleting user-spawned props.

   Rejected alternatives: adding a typed `background_owner` field now would be
   a core domain-model change before the contract is accepted; using display
...
- No `contracts/stage-map/` schema or stage-map spec authoring.
...
- No typed `background_owner` model field until the contract is landed.
...
2. Re-run:
   ...
   ```
3. Confirm `contracts/stage-map/` and `docs/roadmap/single-stage-import.md`
   are still absent or decide whether a newer `origin/main` landed them.
...
3. Validate `map_id`, `source`, `object_id`, and incoming placement tags as
   non-empty slug-like strings before turning them into persistent tags. Reject
   the whole command only for batch-level invalid `map_id`/`source`; emit
   placement diagnostics for invalid per-placement object ids or tags.
...
4. For each valid placement:
   ...
   - set tags to include `background_map`, `map:<map_id>`,
     `source:<source>`, and optional `object:<object_id>`
...
2. Diagnostic source: `stage_import_debug` until the stage-map contract lands a
...
3. Update `crates/shotloom-core/tests/generate_bridge_fixtures.rs` with at
   least one command fixture and one event sequence fixture that includes
   diagnostics plus successful props.
...
2. Spawned props carry `background_map`, `map:<map_id>`,
   `source:stage_import_debug`, and `object:<object_id>` tags.
...
6. Invalid placement tag/object id emits a diagnostic and does not mutate that
   placement.
7. Existing `spawn_prop_from_asset_*` tests still pass and still select/promote
   tools for the single-spawn path.
8. Batch command does not emit `SelectionChanged` or `ActiveToolChanged`.
9. Render spawn failure leaves no partial persisted prop models and no leftover
...
- [ ] Spawned background props include stable ownership tags for STL-424.
...
Manual repro after STL-425 wires UI buttons:
...
- Do not enable Stage Import UI buttons; STL-425 owns dispatch wiring.
- Do not implement clear-all or delete props by asset id; STL-424 owns
  ownership-filtered removal.
- Do not add a typed `background_owner` field until the stage-map contract is
  landed.
...
- STL-424 clear-all command can filter on `background_map` and `map:<map_id>`
  tags.
- STL-425 debug panel can enable buttons and dispatch fixed map payloads.
- A future contract PR can replace tags with typed `background_owner` metadata
  once the stage-map schema is accepted.
