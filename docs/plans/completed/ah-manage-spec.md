...
Create a general agent-hub skill for managing specs as durable work contracts.
...
The working name is `ah-manage-spec`.

## Problem

agent-hub now uses specs as the normal pre-work contract, but the workflow is
spread across several specialized skills and informal chat patterns:

| Existing workflow | Strength | Gap for agent-hub |
|-------------------|----------|------------------|
...
3. Reuse existing spec-related skills when they fit instead of duplicating
   their logic.
4. Keep agent-hub operational specs under `docs/plans/`, using lifecycle
...
`ah-manage-spec` is an orchestrator:
...
`ah-authoring` does not exist yet. The implementation batch must either add a
`ah-authoring` context profile or add a temporary routing metadata exemption
with a review date.
...
Milestone CRUD belongs to [ah-manage-milestone.md](ah-manage-milestone.md).
...
| agent-hub policy, architecture, path, validator, skill/rule/standard work | agent-hub operational spec | use `ah-manage-spec` templates |
...
General agent-hub operational specs should use this structure:
...
`ah-manage-spec` should reference existing skills as follows:
...
| `ah-make-skill` | implementing or changing spec-management skill | skill naming, frontmatter, routing metadata |
| `ah-resolve-doc-path` | spec must resolve repo or Obsidian paths | path lookup instead of hardcoded paths |
...
`ah-manage-milestone` is a planned sibling skill, specified in
[ah-manage-milestone.md](ah-manage-milestone.md).

| Planned skill | Coordinate when | Use for |
|---------------|-----------------|---------|
| `ah-manage-milestone` | a spec must attach to or update a milestone | milestone document lifecycle and link integrity |
...
```bash
test -f agent/skills/ah-manage-spec/SKILL.md
...
1. `agent/skills/ah-manage-spec/SKILL.md` exists and stays under the skill
   body size target from `ah-make-skill`.
2. Long templates live in `references/`.
3. The skill can create a agent-hub operational spec with intake.
...
10. The final docs explain when to use `ah-manage-spec` versus existing
...
1. Create `agent/skills/ah-manage-spec/SKILL.md`.
...
Status: implemented in `agent/skills/ah-manage-spec/`.

### Batch C: Pilot The Skill

1. Use the new skill to update `docs/plans/active/ah-architecture-hardening.md`.
...
| TODO | Timing | Reason |
|------|--------|--------|
| Rename or move existing `docs/plans/` files into a lifecycle structure | after `ah-manage-spec` is implemented and piloted | keep this spec implementation focused; use the new skill as the migration controller |
...
| Should `ah-authoring` profile be added now? | yes, unless validator design prefers an exemption |
