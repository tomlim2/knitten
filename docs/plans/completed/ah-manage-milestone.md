...
Create a general agent-hub skill for managing Markdown milestones as durable
umbrella records for multi-spec outcomes.

The working name is `ah-manage-milestone`.

## Problem

agent-hub now has a milestone folder and an initial milestone contract, but there
...
```text
agent/skills/ah-manage-milestone/
  SKILL.md
  references/
...
`ah-manage-spec` and `ah-manage-milestone` should cooperate:

| Scenario | Owning skill |
|----------|--------------|
| Create or update one spec | `ah-manage-spec` |
| Group specs under a milestone | `ah-manage-milestone` |
| Add `milestone:` while creating a spec | `ah-manage-spec`, then call milestone attach rules |
| Review one spec | `ah-manage-spec` |
| Review umbrella progress | `ah-manage-milestone` |
...
1. `agent/skills/ah-manage-milestone/SKILL.md` exists.
...
1. Land this spec.
2. Review it against the existing milestone index and `ah-manage-spec`.
3. Patch missing invariants before implementing the skill.

### Batch B: Skill Skeleton

Status: implemented in `agent/skills/ah-manage-milestone/`.
