...
| Schema / serialization compatibility | yes | PR #370 defines Stage command/event/DTO wire shape and TS mirrors. | Treat STL-478 as the canonical owner; STL-479 changes runtime behavior only. Any payload rename or DTO field change stops as out of scope. | `cargo test -p shotloom-core --test generate_bridge_fixtures` and editor bridge tests remain green; diff review shows no protocol shape churn. |
