...
- `node scripts/resolve-output.mjs --skill=report-finding --name=test --create`
...
- `node scripts/resolve-output.mjs --skill=draft-spec --name=test`
...
- `report-finding` has a hub-owned operational findings path.
...
- Add `response-json`.
- Replace `finding-json` for `report-finding` with
...
- Add `--target-root` guidance to `report-finding`.
