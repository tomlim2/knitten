...
| `implement` | Code or document implementation. | Changed files plus validation evidence. |
...
Review findings loop back to `implement`. PR review comments also loop
back through `implement` unless they are purely response-only items.
...
- Use `implement` for code/doc fixes from PR feedback.
...
- `report-finding` records recurring workflow or system issues discovered
...
| `draft-spec` | task purpose, references, chosen direction | draft spec with explicit input/output and acceptance criteria | `ah-prepare-work` |
...
| `ah-implement-plan` | reviewed spec and design plan | changed files plus validation evidence | `implement` |
| `ah-apply-review-fixes` | accepted findings and target artifact | updated artifact plus fix summary | `ah-prepare-work`, `implement` |
...
| `report-finding` | recurring issue, failed assumption, or workflow gap | structured finding record and suggested next action | `ah-review-work`, `ah-manage-pr`, `ah-wrapup-work` |
...
| Umbrella | Calls |
|----------|-------|
| `ah-prepare-work` | `ah-gather-references`, `ah-organize-references`, `ah-brainstorm-approaches`, `draft-spec`, `ah-add-design-plan`, `ah-review-spec`, `ah-apply-review-fixes` |
| `implement` | `ah-implement-plan` for first implementation, `ah-apply-review-fixes` when fixing review findings |
...
| `ah-wrapup-work` | `report-finding` for recurring workflow issues, `ah-manage-milestone` when milestone state changes |
