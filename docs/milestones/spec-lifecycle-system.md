...
3. `agent/skills/caol-manage-milestone/SKILL.md` exists and can attach specs to
   milestone docs.
4. High-risk specs can persist intake under `docs/briefings/specs/`.
5. Specs can point to a milestone through frontmatter.
6. Milestone progress can be reviewed from Markdown without opening GitHub,
   GitLab, Linear, or another tracker.
7. `docs/plans/` rename/move work has a separate migration spec before it runs.
8. Validator support covers milestone file naming and later covers milestone
   link consistency.
